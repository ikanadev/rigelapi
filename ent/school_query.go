...
func (sq *SchoolQuery) FirstIDX(ctx context.Context) string {
...
func (sq *SchoolQuery) OnlyIDX(ctx context.Context) string {
...
func (sq *SchoolQuery) IDsX(ctx context.Context) []string {
	ids, err := sq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (sq *SchoolQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*School, error) {
	var (
		...
		loadedTypes = [2]bool{
			sq.withClasses != nil,
			sq.withMunicipio != nil,
		}
	)
	if sq.withMunicipio != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withClasses; query != nil {
		if err := sq.loadClasses(ctx, query, nodes,
			func(n *School) { n.Edges.Classes = []*Class{} },
			func(n *School, e *Class) { n.Edges.Classes = append(n.Edges.Classes, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withMunicipio; query != nil {
		if err := sq.loadMunicipio(ctx, query, nodes, nil,
			func(n *School, e *Municipio) { n.Edges.Municipio = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SchoolQuery) loadClasses(ctx context.Context, query *ClassQuery, nodes []*School, init func(*School), assign func(*School, *Class)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*School)
	for i := range nodes {
	...
	for _, n := range neighbors {
		fk := n.school_classes
		if fk == nil {
			return fmt.Errorf(`foreign-key "school_classes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (sq *SchoolQuery) loadMunicipio(ctx context.Context, query *MunicipioQuery, nodes []*School, init func(*School), assign func(*School, *Municipio)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*School)
	for i := range nodes {
		if nodes[i].municipio_schools == nil {
			continue
		}
		fk := *nodes[i].municipio_schools
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "municipio_schools" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
