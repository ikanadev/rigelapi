...
func (sq *SubjectQuery) QueryClasses() *ClassQuery {
...
func (sq *SubjectQuery) IDs(ctx context.Context) ([]string, error) {
	var ids []string
...
func (sq *SubjectQuery) WithClasses(opts ...func(*ClassQuery)) *SubjectQuery {
	query := &ClassQuery{config: sq.config}
...
func (sq *SubjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subject, error) {
	...
	if query := sq.withClasses; query != nil {
		if err := sq.loadClasses(ctx, query, nodes,
			func(n *Subject) { n.Edges.Classes = []*Class{} },
			func(n *Subject, e *Class) { n.Edges.Classes = append(n.Edges.Classes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubjectQuery) loadClasses(ctx context.Context, query *ClassQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *Class)) error {
	...
	query.Where(predicate.Class(func(s *sql.Selector) {
		s.Where(sql.InValues(subject.ClassesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.subject_classes
		if fk == nil {
			return fmt.Errorf(`foreign-key "subject_classes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "subject_classes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
