...
func (sq *ScoreQuery) QueryActivity() *ActivityQuery {
...
func (sq *ScoreQuery) QueryStudent() *StudentQuery {
	query := &StudentQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ScoreQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (sq *ScoreQuery) FirstIDX(ctx context.Context) string {
...
func (sq *ScoreQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (sq *ScoreQuery) OnlyIDX(ctx context.Context) string {
...
func (sq *ScoreQuery) IDs(ctx context.Context) ([]string, error) {
	var ids []string
...
func (sq *ScoreQuery) IDsX(ctx context.Context) []string {
	ids, err := sq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (sq *ScoreQuery) Clone() *ScoreQuery {
	...
	return &ScoreQuery{
		...
		withStudent:  sq.withStudent.Clone(),
...
func (sq *ScoreQuery) WithActivity(opts ...func(*ActivityQuery)) *ScoreQuery {
	query := &ActivityQuery{config: sq.config}
...
func (sq *ScoreQuery) WithStudent(opts ...func(*StudentQuery)) *ScoreQuery {
	query := &StudentQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withStudent = query
	return sq
}
...
func (sq *ScoreQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Score, error) {
	...
	if sq.withActivity != nil || sq.withStudent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withActivity; query != nil {
		if err := sq.loadActivity(ctx, query, nodes, nil,
			func(n *Score, e *Activity) { n.Edges.Activity = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withStudent; query != nil {
		if err := sq.loadStudent(ctx, query, nodes, nil,
			func(n *Score, e *Student) { n.Edges.Student = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ScoreQuery) loadActivity(ctx context.Context, query *ActivityQuery, nodes []*Score, init func(*Score), assign func(*Score, *Activity)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Score)
	for i := range nodes {
		if nodes[i].activity_scores == nil {
			continue
		}
		fk := *nodes[i].activity_scores
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "activity_scores" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *ScoreQuery) loadStudent(ctx context.Context, query *StudentQuery, nodes []*Score, init func(*Score), assign func(*Score, *Student)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Score)
	for i := range nodes {
		if nodes[i].student_scores == nil {
			continue
		}
		fk := *nodes[i].student_scores
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "student_scores" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
