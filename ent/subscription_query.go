...
func (sq *SubscriptionQuery) QueryTeacher() *TeacherQuery {
...
func (sq *SubscriptionQuery) QueryYear() *YearQuery {
	query := (&YearClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubscriptionQuery) WithTeacher(opts ...func(*TeacherQuery)) *SubscriptionQuery {
	query := (&TeacherClient{config: sq.config}).Query()
...
func (sq *SubscriptionQuery) WithYear(opts ...func(*YearQuery)) *SubscriptionQuery {
	query := (&YearClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(subscription.FieldMethod).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Subscription.Query().
//		Select(subscription.FieldMethod).
//		Scan(ctx, &v)
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if sq.withTeacher != nil || sq.withYear != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withTeacher; query != nil {
		if err := sq.loadTeacher(ctx, query, nodes, nil,
			func(n *Subscription, e *Teacher) { n.Edges.Teacher = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withYear; query != nil {
		if err := sq.loadYear(ctx, query, nodes, nil,
			func(n *Subscription, e *Year) { n.Edges.Year = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubscriptionQuery) loadTeacher(ctx context.Context, query *TeacherQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Teacher)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Subscription)
	for i := range nodes {
		if nodes[i].teacher_subscriptions == nil {
			continue
		}
		fk := *nodes[i].teacher_subscriptions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "teacher_subscriptions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SubscriptionQuery) loadYear(ctx context.Context, query *YearQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Year)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Subscription)
	for i := range nodes {
		if nodes[i].year_subscriptions == nil {
			continue
		}
		fk := *nodes[i].year_subscriptions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "year_subscriptions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
