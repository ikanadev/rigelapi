...
// DptoDelete is the builder for deleting a Dpto entity.
type DptoDelete struct {
	config
	hooks    []Hook
...
func (dd *DptoDelete) Where(ps ...predicate.Dpto) *DptoDelete {
...
func (dd *DptoDelete) ExecX(ctx context.Context) int {
	n, err := dd.Exec(ctx)
	if err != nil {
		panic(err)
	}
...
func (ddo *DptoDeleteOne) Exec(ctx context.Context) error {
	n, err := ddo.dd.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{dpto.Label}
	default:
		return nil
	}
}
